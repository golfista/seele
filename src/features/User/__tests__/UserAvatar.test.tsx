...
describe('UserAvatar', () => {
  describe('enable Auth', () => {
    ...
    it('should show Seele and default avatar when the user is not logged in and enable auth', () => {
      act(() => {
        useUserStore.setState({ enableAuth: () => true, isSignedIn: false, user: undefined });
      });

      render(<UserAvatar />);
      expect(screen.getByAltText('Seele')).toBeInTheDocument();
      expect(screen.getByAltText('Seele')).toHaveAttribute('src', DEFAULT_USER_AVATAR_URL);
    });
  });

  describe('disable Auth', () => {
    it('should show Seele and default avatar when the user is not logged in and disabled auth', () => {
      enableAuth = false;
      act(() => {
