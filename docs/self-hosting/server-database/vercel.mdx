---
title: Deploy Seele with database on Vercel
description: >-
  Learn how to deploy Seele with database on Vercel with ease, including:
...
This article will detail how to deploy the server database version of Seele on Vercel, including: 1) database configuration; 2) identity authentication service configuration; 3) steps for setting up the S3 storage service.
...
You need to obtain the access key for S3 so that the Seele server has permission to access the S3 storage service. In R2, you can configure the access key in the account details:
...
After completing the steps above, the configuration of the server-side database should be done. Next, we can deploy Seele to Vercel and then visit your Vercel link to verify if the server-side database is working correctly.
