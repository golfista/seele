---
title: Seele Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Seele Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Seele image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Seele Container

Assuming the Seele container is named `lobe-chat`, use the following commands to stop and remove the currently running Seele container:
...
### Pull the Latest Seele Image

Use the following command to pull the latest Docker image for Seele:
...
Redeploy the Seele container using the newly pulled image:
...
No need to worry, you won't. All of Seele's chat records are stored in your local browser. Therefore, when redeploying Seele using Docker, your chat records will not be lost.
